# -*- coding: utf-8 -*-
"""
Created on Wed Aug 19 15:14:38 2015
...
class Worker:
    def __init__(self, entry, rep):
        ...
        #Threshold for worker to rate the requestor 1
        self.high_threshold = random.randint(60,90)
        #Threshold for worker to rate the requestor a 0.33
        self.mid_threshold = random.randint(21,50)
        #Threshold for worker to rate the requestor a -0.33, below would be -1
    ...
    def rate_Requestor(self, requestor):
        ...
        if rep_requstor > self.high_threshold:
            requestor.rate(1.0)
            if requestor not in self.like_list:
                self.like_list.append(requestor)
        elif rep_requstor > self.mid_threshold:
            requestor.rate(0.33)
            if requestor not in self.like_list:
                self.like_list.append(requestor)
...
class Requestor:    
    ...
    def rate_Worker(self, worker):
        ...
        if rep_worker > self.high_threshold:
            worker.rate(1.0)
            if worker not in self.like_list:
                self.like_list.append(worker)            
        elif rep_worker > self.mid_threshold:
            worker.rate(0.33)
            if worker not in self.like_list:
                self.like_list.append(worker)
...
#        #Gather workers that have the ability to complete the task
        ...
        worker_list = [worker_ for worker_ in worker_list if worker_.get_ability() >= self.job.get_skill()]
